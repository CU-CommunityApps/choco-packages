...
# SolidWorks 2026
$solid = "Registry::\HKCU\Software\SolidWorks\SolidWorks 2026"
If (!(Test-Path $solid\Security)){New-Item -Path $solid\Security -force}
New-ItemProperty -Path $solid\Security -Name "EULA Accepted 2026 SP3.2 $env:ComputerName $env:UserName" -Value "Yes" -PropertyType "string" -force
...
$composer = "Registry::\HKCU\Software\Dassault Systemes\Composer\7.13\Preferences"
...
New-ItemProperty -Path $composer -Name "EulaAccepted 2026 7.13.4.26389 $env:ComputerName $env:Username" -Value "1" -PropertyType "DWORD" -Force
# Composer Popups
New-ItemProperty -Path $composer -Name "Composer.EulaAccepted" -Value "0" -PropertyType "DWORD" -Force
New-ItemProperty -Path $composer -Name "Composer.EulaAccepted 2026 7.13.4.26389 $env:ComputerName $env:Username" -Value "1" -PropertyType "DWORD" -Force
...
# eDrawings
$eDraw = "Registry::\HKCU\Software\eDrawings\e2026\General"
If (!(Test-Path $eDraw)){New-Item -Path $eDraw -force}
New-ItemProperty -Path $eDraw -Name "ShowLicense 2026 sp03.2 $env:ComputerName $env:UserName" -Value "0" -PropertyType "DWORD" -Force
