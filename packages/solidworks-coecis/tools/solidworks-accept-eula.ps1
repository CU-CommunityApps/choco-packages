...
New-ItemProperty -Path $solid\Security -Name "EULA Accepted 2026 SP5 $env:ComputerName $env:UserName" -Value "Yes" -PropertyType "string" -force
...
New-ItemProperty -Path $composer -Name "Composer.EulaAccepted 2026 2026 7.13.4.26389 $env:ComputerName $env:Username" -Value "1" -PropertyType "DWORD" -Force
...
New-ItemProperty -Path $eDraw -Name "ShowLicense 2026 sp5 $env:ComputerName $env:UserName" -Value "0" -PropertyType "DWORD" -Force
