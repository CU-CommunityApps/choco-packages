...
.EXAMPLE
    ...
    .\troubleshooting.ps1 -S3 somebucketname -App adobedcreader-cornell -Branch adobedcreader
.LINK
...
Param(
    ...
    [string]$S3,
    [Parameter(Mandatory=$true,Position=2,HelpMessage="Application name")]
    [ValidateNotNullOrEmpty()]
    [string]$App,
...
# Variables
$repo = "https://raw.githubusercontent.com/CU-CommunityApps/choco-packages"
$version = ((iwr -URI "$repo/$branch/packages/$App/config.yml").Content | ConvertFrom-Yaml).Version
$output = "$env:USERPROFILE\Desktop\$App.$version.nupkg"
$extract = "$env:USERPROFILE\Desktop\$App.$version"

# If the file doesn't exist, download from S3 bucket
If (!(test-path $output)){
    try{
        $URI = "https://$S3.s3.amazonaws.com/packages/$branch/$App.$version.nupkg"
...
Do{$ans = Read-Host "Extract and troubleshoot $App.$version ?"}Until(($ans.ToLower() -eq "y") -or ($ans.ToLower() -eq "n"))
...
Do{$ans = Read-Host "Test $App.$version ?"}Until(($ans.ToLower() -eq "y") -or ($ans.ToLower() -eq "n"))

If ($ans.ToLower() -eq "y"){
    Invoke-Expression "$extract\chocolateyinstall.ps1 -Mode T -S3 $S3 -App $App" 
}
Else {Write-host "Happy testing!!" -ForegroundColor Green}
